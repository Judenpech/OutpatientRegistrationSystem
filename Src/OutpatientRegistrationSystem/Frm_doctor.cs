...
namespace OutpatientRegistrationSystem
{
    public partial class Frm_doctor : Form
    {
        ...
        private string Mstr = "SELECT d1.No 工号,d1.NAME 姓名,d1.title 职称,d2.NAME 所属科室,d1.specialty 擅长 FROM tb_doctor d1,tb_dept d2 WHERE d1.deptNo=d2.NO;";
        ...
        private void 保存SToolStripButton_Click(object sender, EventArgs e)
        {
            ...
            if (savevalue == 1)
            {
                if (tb_name.Text != "" && tb_no.Text != "" && tb_title.Text != "")
                {
                    try
                    {
                        rowAffected = mysql.getcom("INSERT tb_doctor( No, NAME, title, deptNo, specialty ) SELECT '" + tb_no.Text.Trim() + "','" + tb_name.Text.Trim() + "','"
                            + tb_title.Text.Trim() + "',d.NO,'" + rtb_spec.Text.Trim() + "' FROM dbo.tb_dept d WHERE d.NAME='" + cmb_dept.SelectedItem.ToString() + "';");
                    }
                    catch (SqlException sqlEx)
                    {
                        MessageBox.Show("数据库异常：" + sqlEx.Message, "数据库异常", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            ...
            else
            {
                if (savevalue == 2)
                {
                    if (tb_name.Text != "" && tb_no.Text != "" && tb_title.Text != "")
                    {
                        try
                        {
                            rowAffected = mysql.getcom("UPDATE tb_doctor SET No='" + tb_no.Text.Trim() + "',NAME='" + tb_name.Text.Trim() + "',title='" + tb_title.Text.Trim()
                                + "',specialty='" + rtb_spec.Text.Trim() + "',deptNo=(SELECT deptNo FROM tb_dept WHERE NAME='" + cmb_dept.SelectedItem.ToString()
